...
#include "srrg2_proslam/registration/instances.cpp"

namespace srrg2_proslam {

  bool RawDataPreprocessorStereoProjective::setRawData(BaseSensorMessagePtr msg_) {
    PROFILE_TIME("RawDataPreprocessorStereoProjective::setMeasurement");
    ...
    if (!message_pack) {
      throw std::runtime_error("RawDataPreprocessorStereoProjective::setMeasurement|received "
    ...
    _image_message_left = srrg2_slam_interfaces::extractMessage<srrg2_core::ImageMessage>(
      msg_, param_topic_camera_left.value());
    if (!_image_message_left) {
      throw std::runtime_error("RawDataPreprocessorStereoProjective::setMeasurement|ERROR, "
    ...
    _image_message_right = srrg2_slam_interfaces::extractMessage<srrg2_core::ImageMessage>(
      msg_, param_topic_camera_right.value());
  ...
  void RawDataPreprocessorStereoProjective::compute() {
    PROFILE_TIME("RawDataPreprocessorStereoProjective::compute");
    _status = Error;
    if (!_raw_data || !_image_message_left || !_image_message_right) {
      throw std::runtime_error("RawDataPreprocessorStereoProjective::compute|measurement not set");
    }
    ...
    {
      PROFILE_TIME("RawDataPreprocessorStereoProjective::loadImageFromDisk");
    ...
    {
      PROFILE_TIME("RawDataPreprocessorStereoProjective::extractFeatures");
    ...
    {
      PROFILE_TIME("RawDataPreprocessorStereoProjective::computeMatches");
    ...
    _meas->reserve(stereo_matches.size());
    for (const Correspondence& stereo_match : stereo_matches) {
      ...
      // ds store point
      _meas->emplace_back(stereo_point);
    }
    _status = Ready;
  }
