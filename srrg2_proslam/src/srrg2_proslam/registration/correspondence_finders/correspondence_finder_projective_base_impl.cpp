...
namespace srrg2_proslam {
  ...
  void CorrespondenceFinderProjectiveBase<TransformType_, FixedType_, MovingType_>::compute() {
    ...
    if (ThisType::_fixed_changed_flag || ThisType::_moving_changed_flag || _config_changed) {
      ...
      _local_map_in_sensor_previous.setIdentity();
    ...
    assert(ThisType::_local_map_in_sensor.matrix().norm() > 0);
    ...
    param_projector->setCameraPose(ThisType::_local_map_in_sensor.inverse());
    ...
    if (_current_iteration % param_number_of_solver_iterations_per_projection.value() == 0 ||
      ...
      _local_map_in_sensor_previous = ThisType::_local_map_in_sensor;
      ++_current_iteration;
      ThisType::_postCompute();
      return;
    }
    ...
    const float estimate_change_norm =
      geometry3d::t2tnq(param_projector->cameraPose() * _local_map_in_sensor_previous).norm();
    _local_map_in_sensor_previous = ThisType::_local_map_in_sensor;
    ...
    draw(correspondence_candidate_map_fixed,
         correspondence_candidate_map_moving,
         correspondence_candidates_filtered);
    ...
    //    std::cerr << "estimate: \n" << ThisType::_local_map_in_sensor.matrix() <<
    ...
    if (matching_ratio < ThisType::param_minimum_matching_ratio.value()) {
      ...
      if (_search_radius_pixels < param_maximum_search_radius_pixels.value() ||
        ...
        if (matching_ratio == 0) {
          ...
          ThisType::_local_map_in_sensor.setIdentity();
          _current_iteration = 0;
        } else {
          ++_current_iteration;
        }
