...
namespace srrg2_proslam {
  ...
  void TriangulatorRigidStereo<MeasurementType_, DestType_>::compute() {
    ...
    for (size_t index = 0; index < _stereo_intensity_matches->size(); ++index) {
      ...
      PointType triangulated_point_in_camera;
      triangulated_point_in_camera.status = Invalid;
      ...
      if (x_L - x_R < param_minimum_disparity_pixels.value()) {
        ...
        ThisType::_dest->emplace_back(triangulated_point_in_camera);
        continue;
      }
      ...
      triangulated_point_in_camera.template descriptor() = stereo_match.descriptor();
      triangulated_point_in_camera.template intensity()  = stereo_match.intensity();
      triangulateRectifiedMidpoint(x_L, y_L, x_R, y_R, triangulated_point_in_camera.coordinates());
      triangulated_point_in_camera.status = Valid;
      ThisType::_dest->emplace_back(triangulated_point_in_camera);
    }
  ...
  void TriangulatorRigidStereo<MeasurementType_, DestType_>::initializeBaseline() {
    ...
    if (!_baseline_set) {
      ...
      Isometry3f right_camera_in_left;
      assert(ThisType::_platform &&
             "TriangulatorRigidStereo::initializeBaseline|platform not present");
      _platform->getTransform(
        right_camera_in_left, param_frame_camera_right.value(), param_frame_camera_left.value());
      _baseline_right_in_left = camera_calibration_matrix * right_camera_in_left.translation();
      _b_x                    = _baseline_right_in_left(0);
      _baseline_set           = true;
    }
  }
