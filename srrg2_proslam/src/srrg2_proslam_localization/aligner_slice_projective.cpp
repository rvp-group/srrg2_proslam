...
namespace srrg2_proslam {
  ...
  void AlignerSliceProjective_<FactorType_, FixedType_, MovingType_>::setEstimate(
    const EstimateType& est_) {
    BaseType::setEstimate(est_);
    ThisType::param_finder->setEstimate(ThisType::_robot_in_sensor * est_);
  }

  template <typename FactorType_, typename FixedType_, typename MovingType_>
  AlignerSliceProjective_<FactorType_, FixedType_, MovingType_>::AlignerSliceProjective_() {
  ...
  template <typename FactorType_, typename FixedType_, typename MovingType_>
  void AlignerSliceProjective_<FactorType_, FixedType_, MovingType_>::bindFixed() {
    BaseType::bindFixed();
  ...
  template <typename FactorType_, typename FixedType_, typename MovingType_>
  void AlignerSliceProjective_<FactorType_, FixedType_, MovingType_>::setupFactor() {
    ...
    ThisType::_factor->setCameraMatrix(param_projector->cameraMatrix());
  ...
  template <typename FactorType_, typename FixedType_, typename MovingType_>
  void AlignerSliceStereoProjective_<FactorType_, FixedType_, MovingType_>::bindFixed() {
    BaseType::bindFixed();
  ...
  template <typename FactorType_, typename FixedType_, typename MovingType_>
  void AlignerSliceStereoProjective_<FactorType_, FixedType_, MovingType_>::setupFactor() {
    // ds minimal setup
    BaseType::setupFactor(); // ^
    if (!_baseline_set) {
      srrg2_core::Isometry3f _right_camera_in_left;
      ...
      _baseline_right_in_left_pixelsmeters =
        ThisType::param_projector->cameraMatrix() * _right_camera_in_left.translation();
      _baseline_set = true;
    }
    ThisType::_factor->setBaselinePixels(_baseline_right_in_left_pixelsmeters);
  ...
  template <typename FactorType_, typename FixedType_, typename MovingType_>
  void AlignerSliceProjective_<FactorType_, FixedType_, MovingType_>::draw(
    ...
    if (MovingPointType::Dim >= 2 && ThisType::_moving_slice) {
      ...
      Vector3f* line_buffer    = new Vector3f[buffer_size];

      // ds lines are drawn w.r.t. current robot pose, hence external viewpoint adjustment is needed
      size_t index = 0;
      for (const Correspondence& correspondence : ThisType::_correspondences) {
        ...
        line_buffer[index].setZero();
        ++index;
        line_buffer[index](0) = point_coordinates(0);
        line_buffer[index](1) = point_coordinates(1);
        if (MovingPointType::Dim == 3) {
          line_buffer[index](2) = point_coordinates(2);
        }
        ++index;
      }
      assert(index == buffer_size);
      ...
      canvas_->pushColor();
      canvas_->setColor(Vector3f(1.0, 0, 0));
      canvas_->putLine(buffer_size, line_buffer);
