...
namespace srrg2_proslam {
  ...
  void MergerProjective_<TransformType_, FixedType_, MovingType_>::compute() {
    ...
    assert(BaseType::_moving);
    BaseType::_status = BaseType::Error;

    // ds check if we can skip computation (including all sub classes)
    if (!BaseType::_scene_changed_flag && !BaseType::_moving_changed_flag &&
        !BaseType::_correspondences_changed_flag && !BaseType::_transform_changed_flag) {
    ...
    // ds cache
    const size_t number_of_points_measured = BaseType::_moving->size();
    ...
    if (!BaseType::_correspondences || BaseType::_correspondences->empty()) {
      ...
      for (const Correspondence& correspondence : *BaseType::_correspondences) {
        ...
        assert(static_cast<size_t>(correspondence.moving_idx) < BaseType::_moving->size());
        ...
        assert((BaseType::_transform.inverse() * scene_point.coordinates()).z() > 0);
        ...
        const MeasurementPointType& measured_point((*BaseType::_moving)[index_measurement]);
      ...
      if (number_of_merged_points == 0) {
        std::cerr << "MergerProjective::compute|WARNING: all merge attempts failed ("
                  << BaseType::_correspondences->size() << ")" << std::endl;
      }
      const float merge_ratio =
        static_cast<float>(number_of_merged_points) / BaseType::_correspondences->size();
      if (merge_ratio < param_target_merge_ratio.value()) {
        std::cerr << "MergerProjective::compute|WARNING: low merge ratio: " << merge_ratio;
        std::cerr << " (" << number_of_merged_points << "/" << BaseType::_correspondences->size()
                  << ")" << std::endl;
      }
  ...
  void MergerProjective_<TransformType_, FixedType_, MovingType_>::_addPoints(
    ...
    if (BaseType::param_enable_binning.value()) {
      ...
      for (size_t i = 0; i < number_of_points_measured; ++i) {
        // ds compute bin indices in left camera frame
        const MeasurementPointType& point_in_image((*BaseType::_moving)[i]);
      ...
      points_in_image_to_add = std::move(*BaseType::_moving);
    }
    ...
    // assert(points_in_camera_to_add.size() == points_in_image_to_add.size());
    const TransformType_ world_in_tracker(BaseType::_tracker_in_world.inverse());
  ...
  void MergerProjective_<TransformType_, FixedType_, MovingType_>::_initializeLandmark(
    ...
    const TransformType_& tracker_in_world_,
    const TransformType_& world_in_tracker_) const {
    // ds populate global statistics (landmark components)
    point_.statistics().allocate();
    point_.statistics().setState((world_in_tracker_ * point_.coordinates()));
    point_.statistics().setCovariance(Matrix3f::Identity());
    point_.statistics().setIsInlier(true);

    // ds add current measurement to statistics (optionally digested by landmark estimator)
    point_.statistics().addMeasurement(
    ...
    point_.template transformInPlace<srrg2_core::Isometry, TransformType_>(BaseType::_transform);
  }
