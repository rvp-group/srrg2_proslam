...
namespace srrg2_slam_interfaces {
  ...
  void LandmarkEstimatorEKF_<MeasurementVectorType_, LandmarkType_>::setTransforms(
    const EstimateType& tracker_from_world_,
    const EstimateType& scene_from_tracker_) {
    assert(param_filter.value());
    param_filter->setTransition(
      tracker_from_world_.template cast<double>() /*, TODO include proper covariance*/);
    BaseType::setTransforms(tracker_from_world_, scene_from_tracker_);
  }
  ...
  void LandmarkEstimatorEKF_<MeasurementVectorType_, LandmarkType_>::compute() {
    ...
    const Vector_<float, StateDim> coordinates_in_world(BaseType::_world_from_tracker *
    ...
    BaseType::_landmark->coordinates() =
      BaseType::_scene_from_world * coordinates_in_world.template cast<float>();
  }
