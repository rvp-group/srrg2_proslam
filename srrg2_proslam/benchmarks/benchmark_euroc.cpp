#include <srrg_benchmark/slam_benchmark_suite_euroc.hpp>
...
const Vector3f maximum_mean_translation_rmse               = Vector3f(0.5, 0.5, 0.5);
const Vector3f maximum_standard_deviation_translation_rmse = Vector3f(0.5, 0.5, 0.5);
...
int main(int argc_, char** argv_) {
  ...
  std::string configuration_path("./euroc.conf");
  if (!current_source_directory.empty()) {
    configuration_path = current_source_directory + "/../../configurations/euroc.conf";
  }
  ...
  SLAMBenchmarkSuiteSE3Ptr benchamin(new SLAMBenchmarkSuiteEuRoC());

  // ds load complete dataset and ground truth from disk
  benchamin->loadDataset("./messages.json");
  benchamin->loadGroundTruth("./gt.csv");
